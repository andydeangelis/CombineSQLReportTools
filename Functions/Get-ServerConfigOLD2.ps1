...
function Get-ServerConfig
{
        ...
        if ($server -ne $null)
        {
            if (Test-Connection $server -Count 2 -Quiet)
            {
              ...
              if ($isWindows)
              {
                ...
                Write-Host "Server $server information has been received."
  ...
  # $servers | Start-RSJob -ScriptBlock $scriptBlock -ModulesToImport $dbatools -ArgumentList $_ 

  foreach ($server in $ComputerName)
  {
    Start-RSJob -Name "ServerConfigData" -ScriptBlock $scriptBlock -ModulesToImport $dbatools -ArgumentList $server
  }
  
  # Wait for all jobs to finish before continuing.

  get-rsjob -Name "ServerConfigData" | Wait-RSJob -ShowProgress
  
  # Receive all the data from each completed job.

  # $jobs = Get-RSJob

  $ServerConfigResult = @()
  $ServerConfigResult = Get-RSJob -Name "ServerConfigData" | Receive-RSJob
  ...
  # Once the data has been received, clear the job list.
